...
export default function PayrollTipsPage() {
  ...
  async function loadAll() {
    const [cfg, dailyRows, weekRows, employeeRows] = await Promise.all([
      apiFetch('/app-api/payroll/tips/config') as Promise<TipConfig>,
      apiFetch('/app-api/payroll/tips/daily-inputs') as Promise<TipDailyInput[]>,
      apiFetch('/app-api/payroll/tips/weeks') as Promise<TipWeek[]>,
  ...
  async function saveConfig(event: FormEvent<HTMLFormElement>) {
    ...
    await apiFetch('/app-api/payroll/tips/config', {
      method: 'POST',
      body: JSON.stringify({
  ...
  async function saveDailyInput(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    await apiFetch('/app-api/payroll/tips/daily-inputs', {
      method: 'POST',
      body: JSON.stringify({
        date: dailyDate,
  ...
  async function createWeek(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    await apiFetch('/app-api/payroll/tips/weeks', {
      method: 'POST',
      body: JSON.stringify({
  ...
  async function createAdvance(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    await apiFetch('/app-api/payroll/tips/advance', {
      method: 'POST',
      body: JSON.stringify({
  ...
  async function calculateWeek(id: string) {
    await apiFetch(`/app-api/payroll/tips/weeks/${id}/calculate`, { method: 'POST', body: JSON.stringify({}) });
    await loadAll();
  }

  async function lockWeek(id: string) {
    await apiFetch(`/app-api/payroll/tips/weeks/${id}/lock`, { method: 'POST', body: JSON.stringify({}) });
    await loadAll();
  }

  async function markPaid(id: string) {
    await apiFetch(`/app-api/payroll/tips/weeks/${id}/mark-paid`, { method: 'POST', body: JSON.stringify({}) });
    await loadAll();
  }

  async function setDepartmentOverride(weekId: string) {
    if (overrideWeight === '') return;
    await apiFetch(`/app-api/payroll/tips/weeks/${weekId}/department-override`, {
      method: 'POST',
      body: JSON.stringify({
  ...
  return (
    <section className="space-y-5">
      <header>
        <h1 className="text-3xl font-bold">Advanced Tip Engine v2</h1>
